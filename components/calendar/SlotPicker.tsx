...
export default function SlotPicker({ slots, onSelect, className }: SlotPickerProps) {
  ...
  if (available.length === 0) {
    return (
      <div className={clx("px-4 py-3 text-sm text-zinc-500 dark:text-zinc-400", className)}>
  ...
  return (
    <ul className={clx("px-4 py-3 grid grid-cols-2 gap-2", className)}>
      {available.map((slot) => (
        <li key={slot.start}>
          <button
            type="button"
            className={clx(
              ...
              "hover:bg-violet-50 dark:hover:bg-violet-900/25 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring)]"
            )}
