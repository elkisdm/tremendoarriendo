...
/**
 * 🎨 CLI para Generador de SVGs Glass - Hommie Icon System
 * 
 * Interfaz de línea de comandos para procesar glyphs.json y generar SVGs
 * con efectos glass minimal. Soporta procesamiento en lotes y configuración
 * avanzada.
 */
...
USO:
  npx ts-node scripts/icon-gen/cli.ts [OPCIONES]
...
EJEMPLOS:
  # Generación básica
  npx ts-node scripts/icon-gen/cli.ts

  # Con preset oscuro y tamaños específicos
  npx ts-node scripts/icon-gen/cli.ts --preset dark --sizes md,lg

  # Solo validación
  npx ts-node scripts/icon-gen/cli.ts --validate

  # Modo verbose con reporte
  npx ts-node scripts/icon-gen/cli.ts --verbose --report

  # Batch processing
  npx ts-node scripts/icon-gen/cli.ts --batch --pattern "glyphs-*.json"
`);
}
